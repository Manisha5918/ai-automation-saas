...
function Footer() {
  ...
  return (
    <footer ref={footerRef} className="bg-noir border-t border-nocturnal/20 text-mint/60 py-20 font-inter relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-10 md:gap-8 items-start animate-on-scroll ${footerInView ? 'in-view' : ''}`}>
          ...
          <div className="flex flex-col items-start w-full">
            ...
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-2" noValidate>
              <input
                ...
                className={`w-full bg-nocturnal/20 border focus:outline-none text-arctic text-xs px-3 py-2.5 rounded-lg transition-all duration-200 placeholder-mint/30 ${
                  shaking ? 'animate-shake' : ''
                } ${
                  status === 'error'
                    ? 'border-red-500/50 focus:border-red-500'
                    : 'border-nocturnal/50 focus:border-forsythia'
                }`}
              />
              <button
                type="submit"
            ...
            {status === 'success' && (
              <span className="text-[10px] text-mint font-semibold mt-2.5 flex items-center gap-1.5 animate-slide-in-up">
                <span className="text-forsythia">✓</span> Subscription success!
              </span>
            )}

            {status === 'error' && (
              <span className="text-[10px] text-red-400 font-semibold mt-2.5 flex items-center gap-1.5 animate-slide-in-up">
